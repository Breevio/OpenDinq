  ...
  return (
    <div className="workspace-grid">
      <section className="profile-header">
        <div>
          <p className="eyebrow">Local alpha workspace</p>
          ...
          <div className="result-strip profile-meta">
            ...
            <a href={`/discover?q=${encodeURIComponent(workspace.discoverQuery)}`}>Discover preview</a>
          </div>
        </div>
      </section>
      ...
      <section className="tool-panel">
        ...
        <p className="status">Publishing is local-alpha state only. It is not an auth or ownership boundary.</p>
        <div className="actions">
          <button type="button" onClick={() => publish("published")}>Publish profile</button>
          <button type="button" onClick={() => publish("draft")}>Move to draft</button>
        ...
        <div className="claim-group" key={type}>
          ...
          {claims.map((claim) => (
            <div className="claim-review" key={claim.id ?? claim.text}>
              ...
              <small>{Math.round(claim.confidence * 100)}% confidence · {claim.evidence.length} evidence · {claim.status ?? "approved"}</small>
              <EvidenceDrawer evidence={claim.evidence} />
              {claim.id ? (
                <div className="actions">
                  <button type="button" onClick={() => onPatch(claim.id!, { status: "approved" })}>Approve</button>
                  <button type="button" onClick={() => onPatch(claim.id!, { status: "rejected" })}>Reject</button>
                  <button type="button" onClick={() => onPatch(claim.id!, { status: "pending" })}>Mark pending</button>
                </div>
              ) : null}
            </div>
          ))}
    ...
    <section className="tool-panel">
      ...
      <form className="generator-form" onSubmit={(event) => {
        ...
        <div className="field-grid">
          <label>
            Manual note title
            <input name="title" defaultValue="Manual note" />
          </label>
          <label className="span-2">
            Manual note content
            <textarea name="contentMd" defaultValue="Add context that should appear as a card." />
          </label>
        </div>
        <button type="submit">Create manual note card</button>
      </form>
    </section>
  );
}
  ...
  return (
    <article className="profile-card editor-card">
      ...
      <div className="actions">
        ...
        <button type="button" disabled={!canMoveUp} onClick={() => move(-15)}>Move up</button>
        <button type="button" disabled={!canMoveDown} onClick={() => move(15)}>Move down</button>
        {cardId ? <button type="button" onClick={() => onRegenerate(cardId)}>Regenerate</button> : null}
      </div>
    </article>
  );
}
...
function sourceStatus(source: WorkspaceData["profileSources"][number] | PersonProfile["sources"][number]) {
  return "status" in source ? source.status : "imported";
}
