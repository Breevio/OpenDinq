...
export function createToolHandlers(client: OpenDinqApiClient) {
  return {
    ...
    opendinq_agent_search: ({ input }: { input: string }) => client.agentSearch(input),
    opendinq_web_search: ({ input, query }: { input?: string; query?: string }) => client.webSearch(input ?? query ?? ""),
    opendinq_plan_profile_generation: ({ input }: { input: string }) => client.planProfileGeneration(input),
    opendinq_resolve_profile_candidates: ({ input, query }: { input?: string; query?: string }) => client.resolveProfileCandidates(input ?? query ?? ""),
    opendinq_generate_profile_ai: ({ input, reviewPlan }: { input: string; reviewPlan?: boolean }) => client.generateProfileAi(input, reviewPlan),
