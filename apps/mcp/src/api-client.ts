export type OpenDinqApiClient = {
  ...
  webSearch(input: string): Promise<unknown>;
  planProfileGeneration(input: string): Promise<unknown>;
  resolveProfileCandidates(input: string): Promise<unknown>;
  agentSearch(input: string): Promise<unknown>;
...
export function createOpenDinqApiClient(apiUrl = requiredApiUrl()): OpenDinqApiClient {
  ...
  return {
    ...
    webSearch(input) {
      return request(`${baseUrl}/api/profiles/web-search`, {
        method: "POST",
    ...
    resolveProfileCandidates(input) {
      return request(`${baseUrl}/api/profiles/resolve`, {
        method: "POST",
        body: JSON.stringify({ input })
      });
    },
    agentSearch(input) {
      return request(`${baseUrl}/api/profiles/agent-search`, {
        method: "POST",
        body: JSON.stringify({ input })
      });
    },
