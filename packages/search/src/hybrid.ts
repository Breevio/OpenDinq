...
function hasStrongEnoughEvidence(queryText: string, terms: string[], evidence: SearchEvidenceRef[]): boolean {
  const personLike = looksLikePersonQuery(queryText, terms);
  const matchedTerms = new Set<string>();
  for (const item of evidence) {
    const tokens = tokenize(`${item.title ?? ""} ${item.reason}`);
    for (const term of terms) {
      if (tokens.some((token) => token === term || isNearTokenMatch(term, token))) {
        matchedTerms.add(term);
      }
    }
  }

  if (matchedTerms.size >= Math.min(2, terms.length)) {
    if (!personLike) {
      return true;
    }
    return terms.slice(0, 2).some((term) => matchedTerms.has(term));
  }
  if (personLike) {
    return terms.slice(0, 2).some((term) => matchedTerms.has(term));
  }
  return evidence.some((item) => item.type === "person") && matchedTerms.size >= 1;
}

function looksLikePersonQuery(queryText: string, terms: string[]): boolean {
  return /\b[A-Z][A-Za-z.'-]+\s+[A-Z][A-Za-z.'-]+\b/.test(queryText) || terms.length >= 4 && !KNOWN_NON_PERSON_START_TERMS.has(terms[0] ?? "") && !KNOWN_NON_PERSON_START_TERMS.has(terms[1] ?? "");
}

const KNOWN_NON_PERSON_START_TERMS = new Set(["ai", "agent", "agents", "systems", "programming", "open", "source", "product", "design", "language", "model", "typescript", "mcp", "rust", "python"]);
...
function matchedCards(document: PersonSearchDocument, evidence: SearchEvidenceRef[]) {
  ...
  return document.cards?.filter((card) => (card.visibility === undefined || card.visibility === "public") && ((card.id && ids.has(card.id)) || titles.has(card.title))).slice(0, 3);
}
