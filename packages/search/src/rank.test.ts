...
describe("parseSearchQuery", () => {
  ...
  it("removes product request words so discovery ranks the subject, not generic profile cards", () => {
    expect(parseSearchQuery("Research elonmusk and return profile cards.").terms).toEqual(["elonmusk"]);
    expect(parseSearchQuery("我要搜索 elon musk，看下他的 profile cards").terms).toEqual(["elon", "musk"]);
  });
});
...
describe("rule-based people search", () => {
  ...
  it("does not score rejected claims or non-public cards", async () => {
    const hiddenOnly: PersonSearchDocument[] = [{
      person: { handle: "hidden", displayName: "Hidden Match" },
      artifacts: [],
      cards: [
        { id: "hidden-card", type: "skills", title: "Kubernetes", contentMd: "Kubernetes", visibility: "hidden" },
        { id: "private-card", type: "skills", title: "Kubernetes", contentMd: "Kubernetes", visibility: "private" }
      ],
  ...
  it("matches generated profiles by handle when the query includes agent request boilerplate", async () => {
    const docs: PersonSearchDocument[] = [
      {
        person: { handle: "elonmusk", displayName: "elonmusk", publicStatus: "published" },
        artifacts: [{ id: "repo-elon", type: "repo", title: "elonmusk/profile-index", description: "Public profile evidence", metadata: {} }]
      },
      {
        person: { handle: "demo-product-designer", displayName: "Demo Product Designer", publicStatus: "published" },
        artifacts: [],
        cards: [{ id: "card-demo", type: "summary", title: "Profile cards", contentMd: "Generated profile cards for a product designer." }]
      }
    ];

    const results = await import("./index.js").then(({ hybridSearchPeople }) => hybridSearchPeople("Research elonmusk and return profile cards.", docs));

    expect(results[0]?.person.handle).toBe("elonmusk");
    expect(results.map((result) => result.person.handle)).not.toContain("demo-product-designer");
  });

  it("ranks near-miss person name matches above broad content matches", async () => {
    const docs: PersonSearchDocument[] = [
      {
        person: { handle: "andrej-karpathy", displayName: "Andrej Karpathy", headline: "LLM education researcher", publicStatus: "published" },
        artifacts: [{ id: "paper-karpathy", type: "paper", title: "Neural networks education", description: "Teaching deep learning and language models", metadata: {} }]
      },
      {
        person: { handle: "demo-systems-maintainer", displayName: "Demo Systems Maintainer", publicStatus: "published" },
        artifacts: [{ id: "repo-demo", type: "repo", title: "open-source-llm-runtime", description: "Open source LLM infrastructure and education examples", metadata: {} }]
      }
    ];

    const results = await import("./index.js").then(({ hybridSearchPeople }) => hybridSearchPeople("Andrej Karpaty open source llm education", docs));

    expect(results[0]?.person.handle).toBe("andrej-karpathy");
  });

  it("filters broad content matches when identity terms have no evidence", async () => {
    const docs: PersonSearchDocument[] = [
      {
        person: { handle: "demo-product-designer", displayName: "Demo Product Designer", publicStatus: "published" },
        artifacts: [{ id: "artifact-demo", type: "note", title: "startup-onboarding-system", description: "Public onboarding source", metadata: {} }],
        cards: [{ id: "card-demo", type: "summary", title: "Profile cards", contentMd: "Generated public profile cards for a product designer." }]
      },
      {
        person: { handle: "demo-ml-researcher", displayName: "Demo ML Researcher", publicStatus: "published" },
        artifacts: [{ id: "paper-demo", type: "paper", title: "Q-learning overview", description: "Research note", metadata: {} }]
      }
    ];

    const results = await import("./index.js").then(({ hybridSearchPeople }) => hybridSearchPeople("Zorblatt Q. Unmade public source profile cards", docs));

    expect(results).toHaveLength(0);
  });

  it("does not return content-only matches for lower-case person-name typo queries", async () => {
    const docs: PersonSearchDocument[] = [
      {
        person: { handle: "demo-systems-maintainer", displayName: "Demo Systems Maintainer", publicStatus: "published" },
        artifacts: [{ id: "repo-demo", type: "repo", title: "open-source-llm-runtime", description: "Open source LLM infrastructure and education examples", metadata: { topics: ["open-source", "systems"] } }],
        cards: [{ id: "card-demo", type: "summary", title: "Demo Systems Maintainer profile", contentMd: "Open source LLM runtime education examples." }]
      }
    ];
