...
import { isNearTokenMatch, parseSearchQuery, tokenize } from "./query.js";
import type {
  MatchedSignals,
  ParsedSearchQuery,
...
export function searchPeople(queryText: string, documents: PersonSearchDocument[]): RankedSearchResult[] {
  ...
  return rankPeople(parsedQuery, documents)
    .filter((result) => result.score > 0 && result.evidence.length > 0 && hasStrongEnoughMatch(parsedQuery, result))
...
export function explainMatch(person: SearchPerson, matchedSignals: MatchedSignals): string {
  ...
  if (matchedSignals.personMatches.length > 0) {
    reasons.push(`matched profile identity for ${joinReadable(matchedSignals.personMatches)}`);
  }
...
function collectMatchedSignals(query: ParsedSearchQuery, document: PersonSearchDocument): MatchedSignals {
  ...
  const identityTokens = tokenize(`${document.person.handle} ${document.person.displayName}`);

  for (const term of query.terms) {
    if (identityTokens.some((token) => token === term || isNearTokenMatch(term, token))) {
      personMatches.add(formatMatchedTerm(term));
    }
  }
  ...
  if (personMatches.size > 0) {
    evidence.push(personEvidence(document.person, `Matched profile identity for ${joinReadable([...personMatches])}.`));
  }

  return {
    personMatches: [...personMatches].sort(),
...
function scoreSignals(signals: MatchedSignals): number {
  const personMatch = clamp(signals.personMatches.length / 2);
...
function hasStrongEnoughMatch(query: ParsedSearchQuery, result: RankedSearchResult): boolean {
  const matchedTerms = new Set<string>();
  for (const evidence of result.evidence) {
    const text = tokenize(`${evidence.title ?? ""} ${evidence.reason}`);
    for (const term of query.terms) {
      if (text.some((token) => token === term || isNearTokenMatch(term, token))) {
        matchedTerms.add(term);
      }
    }
  }

  if (matchedTerms.size >= Math.min(2, query.terms.length)) {
    return true;
  }
  return result.evidence.some((item) => item.type === "person") && matchedTerms.size >= 1;
}
...
function visibleCards(cards: SearchCard[] | undefined): SearchCard[] {
  return (cards ?? []).filter((card) => card.visibility === undefined || card.visibility === "public");
}
