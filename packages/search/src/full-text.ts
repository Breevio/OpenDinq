...
import { isNearTokenMatch, tokenize } from "./query.js";
...
function scoreDocument(query: ParsedSearchQuery, document: PersonSearchDocument): SearchProviderMatch {
  ...
  const personText = tokenize(
    `${document.person.handle} ${document.person.displayName} ${document.person.headline ?? ""} ${document.person.bio ?? ""} ${document.person.location ?? ""}`
  );
  const personScore = scoreTokens(query, personText, { fuzzy: true });
  if (personScore > 0) {
    matchedFields += 1;
  ...
  document.cards?.filter((card) => card.visibility === undefined || card.visibility === "public").forEach((card, index) => {
...
function scoreTokens(query: ParsedSearchQuery, tokens: string[], options: { fuzzy?: boolean } = {}): number {
  if (tokens.length === 0 || query.terms.length === 0) {
    return 0;
  }

  const tokenSet = new Set(tokens);
  const matchedTerms = query.terms.filter((term) => tokenSet.has(term) || (options.fuzzy && hasNearToken(term, tokens)));
...
function hasNearToken(term: string, tokens: string[]): boolean {
  return tokens.some((token) => isNearTokenMatch(term, token));
}
