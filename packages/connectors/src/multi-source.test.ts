...
describe("OpenAlex connector", () => {
  ...
  it("dedupes OpenAlex works by DOI or normalized title", () => {
    const artifacts = normalizeOpenAlexWorksToArtifacts([
      {
        id: "https://openalex.org/W1",
        display_name: "An Integrated Brain-Machine Interface Platform with Thousands of Channels",
      ...
      },
      {
        ...
        doi: "10.1038/example",
        cited_by_count: 99,
        primary_location: { landing_page_url: "https://doi.org/10.1038/example" }
      },
      {
        id: "https://openalex.org/W3",
        display_name: "Probabilistic Neural Signal Decoding",
        cited_by_count: 3
      },
      {
      ...
      },
      {
        ...
        display_name: "Probabilistic neural signal decoding (Preprint)",
        cited_by_count: 1
      }
    ]);
    ...
    expect(artifacts.map((artifact) => artifact.title)).toEqual([
      ...
      "Probabilistic neural signal decoding"
    ]);
    expect(artifacts[0]?.url).toBe("https://doi.org/10.1038/example");
    expect(artifacts[1]?.metadata).toEqual(expect.objectContaining({ citations: 7 }));
  });
