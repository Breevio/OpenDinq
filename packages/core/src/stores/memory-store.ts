...
export function createMemoryStore(initialProfiles: PersonProfileRecord[] = []): OpenDinqStore {
  ...
  return {
    ...
    async saveProfileSources(handle, sources) {
      ...
      profile.sources = dedupeIdentitySources([
        ...profile.sources,
        ...saved.map((source) => ({
          type: source.type,
          url: source.url ?? "",
          externalId: inferExternalId(source)
...
function normalizeProfile(profile: PersonProfileRecord): PersonProfileRecord {
  return {
    ...
    claims: profile.claims?.map((claim, index) => normalizeClaim(profile.person.handle, claim, index))
  };
}

function dedupeIdentitySources(sources: PersonProfileRecord["sources"]): PersonProfileRecord["sources"] {
  const byKey = new Map<string, PersonProfileRecord["sources"][number]>();
...
function inferExternalId(source: ProfileSourceRecord): string | undefined {
  if (source.type === "github") {
    return source.url?.match(/^https?:\/\/(?:www\.)?github\.com\/([A-Za-z0-9-]{2,39})(?:\/)?$/i)?.[1];
  }
  if (source.type === "openalex") {
    return source.url?.match(/\/(A\d+)$/i)?.[1]?.toUpperCase();
  }
  if (source.type === "orcid") {
    return source.url?.match(/\/([\dX-]+)$/i)?.[1];
  }
  return undefined;
}
...
function normalizeClaim(handle: string, claim: ProfileClaimRecord, index: number): ProfileClaimRecord {
  return {
    ...claim,
    id: claim.id ?? `claim-${handle}-${index}`,
    personId: claim.personId ?? handle,
    status: claim.status ?? "approved"
  };
}
