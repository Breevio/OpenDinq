...
function sanitizePlan(plan: ProfileGenerationPlan, rawInput: string): ProfileGenerationPlan {
  ...
  const subject = manualOnly ? sanitizeManualSubject(plan.subject, rawInput) : plan.subject;
...
function sanitizeManualSubject(subject: ProfileGenerationPlan["subject"], rawInput: string): ProfileGenerationPlan["subject"] {
  const extracted = extractPersonName(rawInput);
  if (!extracted) {
    return subject;
  }
  const displayName = subject.displayName;
  const overlap = displayName ? tokenOverlap(extracted, displayName) : 0;
  const shouldReplace = !displayName
    || displayName.split(/\s+/).length > 5
    || rawInput.toLowerCase().includes(displayName.toLowerCase())
    || overlap < Math.min(2, extracted.split(/\s+/).length);
  if (!shouldReplace) {
    return subject;
  }
  return {
    ...
    handle: slugify(extracted)
  };
}

function tokenOverlap(left: string, right: string): number {
  const rightTerms = new Set(right.toLowerCase().split(/[^a-z0-9]+/).filter(Boolean));
  return left.toLowerCase().split(/[^a-z0-9]+/).filter((term) => rightTerms.has(term)).length;
}
...
function extractPersonName(input: string): string | undefined {
  const profileFor = input.match(/profile for\s+([A-Z][A-Za-z0-9 .'-]{2,80})/i)?.[1];
  if (profileFor) {
    return titleName(cleanPersonName(profileFor));
  }
  const match = input.match(/(?:research|find|search|lookup|for)\s+([A-Z][A-Za-z0-9 .'-]{2,80})/i);
  return titleName(cleanPersonName(match?.[1]));
}

function cleanPersonName(value: string | undefined): string | undefined {
  return value
    ?.replace(/\s+(?:and|return|using|with|look|see|check|profile|profiles|card|cards)\b.*$/i, "")
    .trim();
}
